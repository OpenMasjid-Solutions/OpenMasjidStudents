...
- **A month you voided can be billed again.** It could not be: voiding a bill to correct it left that
  child unbillable for that month for good — by hand and by the nightly job — and the only feedback was
  "Generated 0 invoice(s)", which reads like there was nothing to bill. Any one-off charge that was on the
  voided bill also stopped being owed, silently. Both fixed: the month regenerates, and the charge comes
  back with it.
- **Per-term fee plans are not billed, and the app now says so instead of implying otherwise.** The
  cadence can be chosen and assigned, but invoices are generated by month and nothing raises a per-term
  line — so a madrasah could set a term fee and collect none of it. The fee-plan form now says this when
  you pick "Per term", and the full-year figure leaves those fees out rather than quoting money that will
  not be charged. If you have per-term plans configured, check whether you have been invoicing them.
...
**The 0.51.0 audit** — a full documentation, security, bug and dead-code pass over the repo before
release. Twelve parallel review dimensions, findings adversarially verified, 82 confirmed and applied.

- **Security.** `staff.resetPassword` did not revoke the target account's sessions — the only one of the
  three password-writing paths that did not, so a held cookie kept full API authority for the rest of its
  12-hour life after an admin reset the password, which is often done precisely because an account looks
  compromised. `mustChangePassword` is not a substitute: it steers the browser and gates nothing. It now
  signs that account out, sparing only the caller's own cookie when they reset their own password (the
  `keep` pattern `changePassword` already uses, since this procedure has no self guard and the UI offers
  it on every row).
- The printed statement's **QR code** resolved its address from the install-time env mirror and could fall
  back to the LAN host, so a masjid whose tunnel URL changed printed QRs pointing at an address that no
  longer resolved. It now uses the same resolver as every emailed link (`portalBase`), with the request
  host kept only as the last resort — which is correct for a LAN-only install and wrong for anything else.
- A replayed autopay **success** reset the retry ladder on an already-charged run, so a reconcile pass over
  an old PaymentIntent handed a family mid-decline a fresh three attempts and pushed back the auto-disable.
  The failure path has always guarded on a still-pending run; the success path now does too.
- The **year grid** re-derived an invoice's status instead of using the ledger's rule, and asked "has
  anything been paid?" before "does this cost anything?" — so a fully-waived month read Open in the grid
  while the statement called it Paid. `statusFor` is now exported and used by both; the comment on it
  already named this exact symptom, one file over.
- `assertBillablePeriod` skipped the reserved-name check for a term period, not just the month-spelling
  rule it is meant to exempt — so a term run could be aimed at `carry-in` or at a single charge's own
  invoice. Split into `reservedPeriodError`, which nothing is exempt from.
- Autopay's emails called a saved **bank account** a "card" (`cardLabelFor` read `brand` only, and a
  `us_bank_account` row keeps its institution in `bankName`). Telling a family their card was declined
  when they never gave one is a phone call. One namer now, in `payments/methods.ts` beside `feeKindOf`.
- The parent portal printed a **raw i18n key** after a successful card payment — `paidOptimiztic` with a
  `z`, against a catalog holding `paidOptimistic`. One character, on the screen a parent sees the moment
  their money leaves.
- Household **statements** printed the raw enums `ach` and `carry_in` to parents; the invoice document's
  copy of the same table had both. The **past-due digest** told an office "parent reminders are switched
  off" in the same email that reported the reminders it had just sent by WhatsApp. The **Recorded by**
  column printed the channel word for a card payment instead of the "Automatic" label written for it.
- **Documentation.** Every `.md` file checked against the code. `CLAUDE.md`'s §8 repository tree was
  missing seven modules and two routers and mislocated `public/`; §9's schema list claimed to be complete
  and was one table short; §11.2's wire contract omitted the two 0.51.0 processing-fee fields that §11.3
  and the shipped code both depend on; §17 documented a build step that typechecks (it does not) and a dev
  proxy missing two of the three document prefixes. `docs/FABRIC_BILLING_CONTRACT.md` — the extract four
  repos read — had dropped `students_channel` from the metadata block, undercounted reconciliation's
  channels, and lost the receipt a recorded external payment sends. `PAYMENTS.md` did not know about
  standing payments, quoted a card fee the code disagreed with ($59.79 against the real $60.05 — the
  code's own comment was wrong too, at $59.98), and numbered its Refunds section §13.4, which is
  CLAUDE.md's no-webhook doctrine. `WHATSAPP.md` credited the platform queue with pacing removed in
  platform 0.51.1 and documented a 404 behavior its own next section calls a defect. `DATA_MODEL.md`
  asserted a live gradebook and tracked five assumptions the v0.35.0 pivot closed. `README` claimed an
  admin can force autopay off (never built) and that nobody in the office can override a WhatsApp opt-out
  (they can, since 0.51.0). All eight anchor links in `AUDIT-2026-08-13.md` were broken — GitHub does not
  support `{#id}` heading attributes.
- **Dead code removed:** three unused exports (`isParentEvent`, `cachedWhatsAppStatus`,
  `setTestStudentId`), a DB column nothing has ever read (`users.staff_notes`), two env vars parsed into
  config and never used (`SCHOOL_NAME`, `CURRENCY` — the school name and currency are collected in-app),
  80 lines of dead CSS in `admin.css` (a replaced top-bar layout, `.back-link`, `.fam-card`, `.admin-nav`
  and a phone rule for grid classes nothing uses), and two superseded i18n keys. A third orphaned key was
  the success message for the hard-delete, which is now shown rather than deleted. The print stylesheet
  hid `.scene-bg`, a class the app has never rendered — so the ambient backdrop was not being hidden when
  printing; it is `.scene`.
- Dead rules in the three **ported** stylesheets (48 of them) were found and deliberately left alone:
  those files are kept structurally identical to OpenMasjidOS for re-sync (§15), so the dead rules are
  upstream's. Recorded so they are not re-discovered as a cleanup opportunity.
- A test was a **latent time bomb**: `joinMidYear` asserted that `2026-09` was "still ahead of us", which
  was true until the calendar reached it. It now derives the month from the clock.
...
- WhatsApp delivery checking is now exact rather than approximate: OpenMasjidOS names the specific
  ...
  knowing what to check and going looking.
...
- **Security: changing your password now signs you out on your other devices.** Resetting a
  *forgotten* password always did that; changing a password you still knew did not — and that is the
  version of it people actually reach for, as well as the only one available from inside the app. So a
  parent who signed in on a borrowed phone, forgot to sign out, and later changed their password from
  home precisely because they were worried left that phone signed in for up to another twelve hours.
  **The device you change it on stays signed in; every other one is signed out.** Nothing else about
  signing in changes.
- **Fixed: accepting a parent-portal invite left the family on a blank page.** A parent who opened
  their invite link and set a password was signed in correctly — and then sent somewhere else
  entirely, because the app forgot it was being served under a sub-address when reached over the
  internet. Their account existed and worked, but the first thing they saw said otherwise, so the
  reasonable conclusion was that signing up had failed. This was the first step for every family who
  joins by invite.
- **Fixed: generating a month's invoices got slower every year.** Matching payments to bills was
  re-reading the whole school's payment history once for every child, so the nightly run and the
  Generate button got heavier as the years added up — most noticeably on a Raspberry Pi and at the
  largest madāris. Balances, the ticks on the year grid and the order money lands in are all exactly as
  before.
- **Security: the app no longer ships a mail library it never used.** Sending email moved to
  OpenMasjidOS a while ago, but the old mail component was still being installed into every copy of the
  app, and eight security problems had since been found in it. It was never switched on and could never
  have sent anything, so nobody was at risk — but it had no business being there. A leftover PDF
  component from the old report cards went with it, which makes the download smaller too.
- **Printed family statements are served locked down.** A statement carries every child's Student ID
  and the household's payment history, so the page can no longer load anything from another website or
  send anything to one, and it cannot be embedded in one.
- The release process is harder to tamper with: every build step is now pinned to an exact verified
  version, so the app you install is the app we built. Five shared components were updated to close
  published vulnerabilities, one of them in the part that answers every web request. Nothing about how
  the app looks or behaves changes.
- Housekeeping: all 527 automatic checks — the money and allocation rules, the access walls, the
  version numbers, the alert list — now run before any release can ship, where before they only ever
  ran on a developer's machine.
