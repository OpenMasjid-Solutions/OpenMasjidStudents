...
describe('charge items are a catalog, and applying one snapshots it', () => {
  it('applies an item at its default price and records the label', async () => {
    ...
    const r = await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'item', chargeItemId: item.id } });
  ...
  it('re-pricing or renaming the item does NOT rewrite a charge already applied', async () => {
    const { admin, studentId, familyId } = await seed();
    const item = await admin.billing.chargeItemCreate({ name: 'Qaidah book', defaultAmountCents: 1500 });
    await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'item', chargeItemId: item.id } });
  ...
  it('an item can be re-priced for one application without touching the catalog', async () => {
    ...
    await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'item', chargeItemId: item.id, amountCents: 3000 } });
  ...
  it('a custom one-off needs no item, and a zero charge is refused', async () => {
    const { admin, studentId, familyId } = await seed();
    await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Late fee', amountCents: 2500 } });
    expect((await admin.billing.chargeList({ familyId }))[0]).toMatchObject({ label: 'Late fee', amountCents: 2500 });
    await expect(admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Nothing', amountCents: 0 } })).rejects.toMatchObject({ code: 'BAD_REQUEST' });
  });
});
...
describe('when a charge reaches an invoice', () => {
  it('waits as pending with no invoice, then is picked up by generation', async () => {
    const { admin, studentId, familyId } = await seed();
    await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Registration', amountCents: 5000 }, periodKey: '2026-07' });
  ...
  it('an UNTARGETED charge rides the next invoice generated, whatever its period', async () => {
    const { admin, studentId, familyId } = await seed();
    await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Trip', amountCents: 1000 } });
  ...
  it('lands on an already-open invoice immediately and re-derives its status', async () => {
    ...
    const r = await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Late fee', amountCents: 1500 }, periodKey: '2026-07' });
  ...
  it('refuses to touch a VOID invoice and leaves the charge pending', async () => {
    ...
    const r = await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Book', amountCents: 1000 }, periodKey: '2026-07' });
  ...
  it('a NEGATIVE charge is how a credit is issued', async () => {
    const { admin, studentId, familyId } = await seed();
    await admin.billing.generateFamily({ familyId, periodKey: '2026-07', label: 'Jul' });
    await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Scholarship', amountCents: -2000 }, periodKey: '2026-07' });
...
describe('voiding a charge', () => {
  it('works while pending, and is refused once invoiced', async () => {
    const { admin, studentId, familyId } = await seed();
    const a = await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Book', amountCents: 1000 } });
    ...
    const b = await admin.billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'Late fee', amountCents: 1000 }, periodKey: '2026-07' });
...
describe('bulk apply over the course → class grouping', () => {
  ...
  it('chargeAddBulk charges an entire course from the item side', async () => {
    ...
    const r = await admin.billing.chargeAddBulk({ bill: 'period', source: { kind: 'item', chargeItemId: item.id }, target: { kind: 'course', courseId: course }, periodKey: '2026-07' });
  ...
  it('a bulk target skips withdrawn students', async () => {
    ...
    const r = await admin.billing.chargeAddBulk({ bill: 'period', source: { kind: 'item', chargeItemId: item.id }, target: { kind: 'class', classId: c1 } });
...
describe('walls', () => {
  it('charges are admin + finance only; a parent is refused', async () => {
    ...
    await expect(caller('parent').billing.chargeAdd({ bill: 'period', studentId, source: { kind: 'custom', label: 'X', amountCents: 100 } })).rejects.toMatchObject({ code: 'FORBIDDEN' });
